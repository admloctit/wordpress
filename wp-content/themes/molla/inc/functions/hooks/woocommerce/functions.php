...
if ( ! function_exists( 'molla_loop_product_thumbnail_size' ) ) :
	...
	function molla_loop_product_thumbnail_size() {
		if ( wc_get_loop_prop( 'widget' ) && wc_get_loop_prop( 'type' ) ) { // Product widget
							...
							foreach ( $terms as $term ) {
								if ( in_array( $term->slug, $options, true ) ) {
									...
									if ( in_array( $attribute_name, $pick_tax ) ) {
										...
										// Get selected value.
										$current_key = 'attribute_' . sanitize_title( $attribute_name );
										$selected = isset( $_REQUEST[ $current_key ] ) ? ( $_REQUEST[ $current_key ] == $term->slug ) : false;
...
if ( ! function_exists( 'molla_single_product_image_thumbnail_html' ) ) :
	 ...
	 */
	function molla_single_product_image_thumbnail_html( $html, $id, $layout = -1 ) {
		if ( ! $layout || 'extended' == $layout || 'boxed' == $layout || 'full' == $layout ) {
			return $html;
		}
		$html  = substr( $html, 0, -6 );
		$html .= '<a href="#" class="btn-product-gallery"><i class="icon-arrows"></i></a></div>';
		return $html;
	}
endif;
	...
	function molla_woocommerce_single_product_deal() {
		...
		if ( $product->is_on_sale() ) {
			...
			if ( $end_date ) {
				...
				echo '<div class="deal-container ' . $type . '-type">' . ( 'inline' == $type && $label ? ( '<span class="countdown-title">' . esc_html( $label ) . '</span>' ) : '' ) . '<div class="deal-countdown" data-until=' . esc_attr( $offset ) . ' data-relative="true"></div></div>';
			}
		}
	}
endif;
...
if ( ! function_exists( 'molla_woocommerce_breadcrumb_args' ) ) :
	function molla_woocommerce_breadcrumb_args( $args ) {
		...
		$args['delimiter']   = '<span class="breadcrumb-delimiter"><i class="icon-angle-right"></i></span>';
