...
(function ($) {
	...
	var self = Molla = {
		...
		thumbnailSlide: function ($selector) {
			...
			if ($carousel.length && $outer.length) {
				...
				var $quickview = $carousel.closest('.quickView-content'),
					$thumbnails = $quickview.find('.thumbnails-outer'),
					$gallery_carousel = $quickview.find('.product-gallery-carousel'),
					zoomOptions = {},
				...
				if ($thumbnails.hasClass('owl-carousel')) {
					if (!$thumbnails.find('.woocommerce-product-gallery__image a.active').length) {
						$thumbnails.find('.owl-item:first-of-type .woocommerce-product-gallery__image a').addClass('active');
					}
				} else {
					if (!$thumbnails.find('.woocommerce-product-gallery__image a.active').length) {
						$thumbnails.children('.woocommerce-product-gallery__image:first-of-type').find('a').addClass('active');
					}
				}
				$gallery_carousel.on(
					'click',
				...
				$thumbnails.on(
					...
					function (e) {
						...
						$thumbnails.find('.woocommerce-product-gallery__image a').removeClass('active');
		...
		singleInit: function ($selector) {
			...
			var defer_wc_single = (function () {
				...
				if ($.fn.wc_product_gallery) {
					...
					if ('' == theme.wc_url) {
						...
						).attr('src', theme.wc_url + 'js/frontend/single-product.min.js');
					}
				}
				return deferred.promise();
			})();
			...
			$.when(defer_input_spinner, defer_carousel, defer_elevate, defer_wc_single, defer_wc_variation).done(
				function (e) {
	...
	$(document).on(
		...
		'.variations_form',
		function (e, args) {
			$('.product .product-intro .product-main-image .wp-post-image').attr('data-zoom-image', '');
		}
	)
