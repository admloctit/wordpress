							...
							<div class="woocommerce_attribute_data wc-metabox-content hidden">
								<div class="swatch_type_select">
									...
									<select class="swatch_type" id="swatch_options[<?php echo esc_attr( $attribute->get_name() ); ?>][type]" name="swatch_options[<?php echo esc_attr( $attribute->get_name() ); ?>][type]">
										...
										<option value="image" <?php selected( $swatch_type, 'image' ); ?>><?php esc_html_e( 'Image', 'molla' ); ?></option>
									</select>
									<p style="<?php echo esc_attr( 'image' == $swatch_type ? 'display: none;' : '' ); ?>"><?php esc_html_e( 'This attribute will not be shown as image swatch. However it will replace product image with following uploaded image when clicked.', 'molla' ); ?></p>
								</div>
