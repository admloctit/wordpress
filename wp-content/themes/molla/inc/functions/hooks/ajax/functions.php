	...
	function molla_mobile_menu() {
		...
		if ( $mobile_tab ) :
			...
			foreach ( $mobile_menus as $menu ) :
				...
				<?php $active = false; ?>
					<a class="nav-link" id="<?php echo esc_attr( $menu ); ?>-link" data-toggle="tab" href="#<?php echo esc_attr( $menu ); ?>-tab" role="tab" aria-controls="<?php echo esc_attr( $menu ); ?>-tab" aria-selected="true"><?php echo esc_html( $menu_obj->name ); ?></a>
				</li>
				<?php
			endforeach;
			...
			foreach ( $mobile_menus as $menu ) :
				?>
			<div class="tab-pane fade<?php echo ! $active ? '' : ' show active'; ?>" id="<?php echo esc_attr( $menu ); ?>-tab" role="tabpanel" aria-labelledby="<?php echo esc_attr( $menu ); ?>-link">
				<?php
				wp_nav_menu(
					array(
						'menu'           => $menu,
						'menu_class'     => 'mobile-menu mobile-' . $menu . '-menu',
			...
			wp_nav_menu(
				array(
					'menu'           => $mobile_menus[0],
					'menu_class'     => 'mobile-menu mobile-' . $mobile_menus[0] . '-menu',
					'menu_id'        => 'mobile-' . $mobile_menus[0] . '-menu',
					'lazy'           => false,
					'theme_location' => '',
				)
			);
			...
			echo  'var wc_single_product_params = ' .
				json_encode(
					apply_filters(
						...
						array(
							'i18n_required_rating_text' => esc_attr__( 'Please select a rating', 'molla' ),
